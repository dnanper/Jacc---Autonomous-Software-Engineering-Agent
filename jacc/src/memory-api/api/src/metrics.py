"""
OpenTelemetry metrics instrumentation for Memory API.
...
For SWE-Agent: Uses NoOpMetricsCollector by default (no overhead).
"""
...
class MetricsCollector(MetricsCollectorBase):
    """
    Collector for Memory API metrics.
    ...
    def __init__(self):
        ...
        self.operation_duration = self.meter.create_histogram(
            name="memory_api.operation.duration", description="Duration of Memory API operations in seconds", unit="s"
        )

        # Token usage counters
        self.tokens_input = self.meter.create_counter(
            name="memory_api.tokens.input", description="Number of input tokens consumed", unit="tokens"
        )

        self.tokens_output = self.meter.create_counter(
            name="memory_api.tokens.output", description="Number of output tokens generated", unit="tokens"
        )

        # Operation counter (success/failure)
        self.operation_total = self.meter.create_counter(
            name="memory_api.operation.total", description="Total number of operations executed", unit="operations"
        )
...
def initialize_metrics(service_name: str = "memory-api", service_version: str = "1.0.0"):
    """
    Initialize OpenTelemetry metrics with Prometheus exporter.

    This should be called once during application startup.
    Returns PrometheusMetricReader instance (for accessing metrics endpoint).
    """
    global _meter
...
def create_metrics_collector() -> MetricsCollector:
    ...
    try:
        ...
        logger.warning(f"Failed to create metrics collector: {e}. Using no-op collector.")
