...
class RequestContext(BaseModel):
    ...
    This class carries authentication data from HTTP requests to the
    memory engine operations. It can be extended to include additional
    context like headers, tokens, user info, etc.
    """

    api_key: str | None = Field(default=None, description="API Key for authentication")
    api_key_id: str | None = Field(default=None, description="UUID of the API key used for authentication")
    tenant_id: str | None = Field(default=None, description="Tenant identifier (set by extension after auth)")  
    internal: bool = False
