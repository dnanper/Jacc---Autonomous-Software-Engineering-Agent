...
_global_llm_semaphore = asyncio.Semaphore(DEFAULT_LLM_MAX_CONCURRENT)
...
class LLMProvider:
    ...
    def __init__(
        ...
        Args:
            ...
            seed: Seed for reproducibility (used by Groq).
        """
    ...
    def from_config(cls, config) -> "LLMProvider":
        """
        Create LLM provider from a LLMConfig object.
        
        Args:
            config: LLMConfig object from config.py
    ...
    def from_env(cls) -> "LLMProvider":
        ...
        if not api_key and provider != "ollama":
            raise ValueError(f"{ENV_LLM_API_KEY} environment variable is required")
        base_url = os.getenv(ENV_LLM_BASE_URL, "")
        model = os.getenv(ENV_LLM_MODEL, DEFAULT_LLM_MODEL)

        return cls(provider=provider, api_key=api_key, base_url=base_url or None, model=model, reasoning_effort="low")
