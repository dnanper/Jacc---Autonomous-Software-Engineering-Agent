...
Cross-encoders evaluate query-document pairs to produce relevance scores,
which are used to rerank initial retrieval results.
"""
...
class CrossEncoderModel(ABC):
    ...
    Unlike bi-encoders (embeddings), cross-encoders see both query and document
    together, allowing for more accurate relevance scoring at the cost of speed.
    
    Usage:
        reranker = LocalSTCrossEncoder()
...
def create_cross_encoder(config) -> CrossEncoderModel:
    """
    Create a CrossEncoderModel instance from a configuration object.
    ...
    if provider == "tei":
        if not config.tei_url:
            raise ValueError("tei_url is required when provider is 'tei'")
        return RemoteTEICrossEncoder(base_url=config.tei_url)
    elif provider == "local":
        return LocalSTCrossEncoder(model_name=config.local_model)
    else:
        raise ValueError(f"Unknown reranker provider: {provider}. Supported: 'local', 'tei'")
