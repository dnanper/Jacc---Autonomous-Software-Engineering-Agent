...
def create_embeddings(config) -> Embeddings:
    """
    Create an Embeddings instance from a configuration object.
    
    Args:
        config: EmbeddingsConfig object
    ...
    if provider == "tei":
        ...
        return RemoteTEIEmbeddings(base_url=config.tei_url)
    elif provider == "local":
        return LocalSTEmbeddings(model_name=config.local_model)
    else:
        raise ValueError(f"Unknown embeddings provider: {provider}. Supported: 'local', 'tei'")
